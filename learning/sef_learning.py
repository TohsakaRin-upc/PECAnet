...
import sys
sys.path.append("/content/gdrive/MyDrive/resnet2.0")
sys.path.append("/public/home/G19860020/HYK/resnet2.0")
...
def train(model, dataloader, criterion, optimizer, scheduler, datasetname=None, isckpt=False, epochs=50,
          networkname=None, writer=None, maxent_flag=False, device='cpu', **penalty):
        ...
        for phase in ['trainval', 'test']:
            ...
            for inputs, labels in dataloader[phase]:
                ...
                if global_step % 100 == 1 and writer is not None and phase is 'trainval':
                    ...
                    for name, param in model.named_parameters():
                        writer.add_histogram('params_in_running/' + name, param.data.clone().cpu().numpy(),
            ...
            if writer is not None:
                if phase is 'trainval':
                    ...
                    for name, param in model.named_parameters():
                        writer.add_histogram('params_in_epoch/' + name, param.data.clone().cpu().numpy(),
                                             epoch)  # global_step
                elif phase is 'test':
...
def eval(model, dataloader=None, device='cpu', datasetname=None):
    if not datasetname or datasetname not in ['cubbirds', 'stcars', 'stdogs', 'FGVC_Aircraft', 'nabirds']:
